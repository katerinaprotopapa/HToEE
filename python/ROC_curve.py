...
class ROC:
    ...
    def plotting(self):
        ...
        for variable in list_variables:
            ...
            self.values_bkg, self.bins_bkg, _ = plt.hist(combined_sig_exc_vbf, bins = num_bins, density = normalize, color = 'lightskyblue', label = 'Background',  histtype = 'step', weights = combined_sig_w_exc_vbf)
            self.values_sig, self.bins_sig, _ = plt.hist(vbf_sig, bins = num_bins, density = normalize, color = 'blue', label = 'Signal',  histtype = 'step', weights = scale * vbf_sig_w)
            ...
            name = 'plotting/plots/new/' + variable 
            #fig.savefig(name)
    ...
    def roc_curve(self):
      ...
      plt.plot()
      plt.xlabel('$\epsilon_{vbf}$')
      plt.ylabel('$\epsilon_{rest}$')
      ...
      plt.title(title)
      name = 'plotting/plots/roc/ROC' + variable
      fig.savefig(name)   
...
"""
Notes:   1) curve_fit maybe ?
         2) loop through more variables - generalize code
