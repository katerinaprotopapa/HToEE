...
class BDTHelpers(object):
    ...
    def __init__(self, data_obj, train_vars, train_frac, eq_train=False):
        ...
        self.clf              = xgb.XGBClassifier(objective='binary:logistic', n_estimators=100, 
                                                  eta=0.05, maxDepth=7, min_child_weight=0.1, 
                                                  subsample=0.6, colsample_bytree=1.0, gamma=3)  # Kat and Philipp HP optimization
    ...
    def plot_roc(self, out_tag, analysis = False):
        ...
        if analysis == False:
            roc_fig = self.plotter.plot_roc(self.y_train, self.y_pred_train, self.train_weights, 
                                   self.y_test, self.y_pred_test, self.test_weights, out_tag=out_tag, dijetcentrality_bool =False, analysis = analysis, nn_roc = True)
